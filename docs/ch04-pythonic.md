# 第 4 章：Pythonic 之道 —— 推导式与正则

> **"Brevity is the soul of wit."** > **“简洁是智慧的灵魂。”**
> — _威廉·莎士比亚，《哈姆雷特》 (William Shakespeare, Hamlet)_

---

什么是 **Pythonic**？它不仅是指遵循 PEP 8 格式规范，更是一种追求代码“简洁、明确、可读”的思维方式。

在 Python 社区，如果你写了 C 语言风格的 `for i in range(len(list))`，或者写了 Java 风格的 Getter/Setter，会被认为是不够 Pythonic 的。本章我们将学习 Python 最迷人的语法糖——推导式，以及强大的文本处理工具。

## 4.1 推导式 (Comprehensions)：一行胜十行

如果你还在用 `for` 循环创建一个经过过滤或处理的新列表，那么请停下来。推导式是 Python 的标志性特性，它既高效又易读。

### 4.1.1 列表推导式 (List Comprehension)

**传统写法 (C/JS 风格)**:

```python
numbers = [1, 2, 3, 4, 5]
squared = []
for n in numbers:
    if n % 2 == 0:
        squared.append(n * n)
```

**Pythonic 写法**:

```python
# [表达式 for 变量 in 容器 if 条件]
squared: list[int] = [n * n for n in numbers if n % 2 == 0]
```

这不仅仅是语法糖，它在解释器层面进行了优化（字节码指令更少），通常比等价的 `for` 循环快。

### 📝 TS 开发者便签：Map & Filter

> 列表推导式本质上就是 TS 中的 `.map().filter()` 链式调用的结合体。
>
> - **TS**:
>   ```typescript
>   const squared = numbers.filter((n) => n % 2 === 0).map((n) => n * n);
>   ```
> - **Python**:
>   ```python
>   squared = [n * n for n in numbers if n % 2 == 0]
>   ```
>
> **文化差异**：虽然 Python 也有 `map()` 和 `filter()` 函数，但在 Python 社区中，**推导式**是首选。除非你需要懒加载（使用生成器），否则尽量少写 `map(lambda x: ..., data)`，因为它可读性较差。

### 4.1.2 字典与集合推导式

不要只盯着列表，字典和集合也能推导。

```python
users = [("Alice", 101), ("Bob", 102), ("Charlie", 103)]

# 字典推导式：快速构建 lookup table
# {key_expr: value_expr for ...}
user_map: dict[str, int] = {name: uid for name, uid in users}
# 结果: {'Alice': 101, 'Bob': 102, ...}

# 集合推导式：快速去重并处理
# {value_expr for ...}
raw_tags = [" python ", "Code", "PYTHON", " code "]
unique_tags: set[str] = {t.strip().lower() for t in raw_tags}
# 结果: {'python', 'code'}
```

## 4.2 生成器表达式 (Generator Expression) —— 内存的救星

推导式虽好，但它有一个缺点：**渴望执行 (Eager Evaluation)**。
`[x for x in range(10000000)]` 会立即在内存中创建一千万个整数，瞬间撑爆内存。

如果你只是想遍历它，或者计算它的总和，只需把**中括号 `[]` 换成小括号 `()`**。

```python
from collections.abc import Iterator

# 这不会立即计算，而是一个生成器对象，占用的内存微乎其微
# 这里的类型注解是 Iterator
large_stream: Iterator[int] = (x * x for x in range(1_000_000))

# 配合 sum, any, all 等函数使用非常优雅
# 注意：当它是函数唯一参数时，可以省略一层括号
total = sum(x * x for x in range(100))
```

### 📝 TS 开发者便签：Lazy Evaluation

> - **List Comp `[]`**: 类似 TS 的 `Array.map`，立即生成数组。
> - **Gen Exp `()`**: 类似 TS 的 Generator 函数或 Iterator。它只在你调用 `next()` 或遍历时才计算下一个值。
>
> **最佳实践**：如果列表很大，且你只需要遍历一次，永远优先使用生成器表达式。

## 4.3 正则表达式 (Regex) 与类型注解

Python 的正则模块是 `re`。虽然正则语法本身各语言都差不多，但在 Python 中结合类型注解使用时，有一些优雅的姿势。

### 4.3.1 预编译与 Raw String

在 Python 中写正则，永远要使用 `r"..."` (Raw String)。它告诉 Python 解释器不要转义反斜杠 `\`。
如果不加 `r`，匹配数字 `\d` 就得写成 `\\d`，匹配反斜杠本身就得写成 `\\\\`，简直是地狱。

```python
import re

# 1. 预编译正则 (Pre-compile)
# 这是一个良好的习惯，提升性能，且能提前发现正则语法错误
# 模式：匹配一个或多个数字
DIGIT_PATTERN: re.Pattern = re.compile(r"\d+")

def extract_ids(text: str) -> list[int]:
    # findall 返回 list[str]
    matches: list[str] = DIGIT_PATTERN.findall(text)
    return [int(m) for m in matches] # 结合列表推导式
```

### 4.3.2 命名分组与类型

Python 的正则支持命名分组 `(?P<name>...)`，这比通过索引 `group(1)` 访问要健壮得多。

```python
def parse_email(text: str) -> dict[str, str] | None:
    # 定义正则：提取 user 和 domain
    pattern = re.compile(r"(?P<user>[\w\.]+)@(?P<domain>[\w\.]+)")

    # match 返回 Match 对象或 None
    match: re.Match | None = pattern.search(text)

    if not match:
        return None

    # groupdict 直接把命名分组转为字典
    return match.groupdict()

# 测试
print(parse_email("Contact: alice.dev@google.com"))
# {'user': 'alice.dev', 'domain': 'google.com'}
```

### 📝 TS 开发者便签：Regex 写法

> - **TS**: `/pattern/g` 字面量。JS 引擎会自动缓存字面量正则。
> - **Python**: 没有 `/.../` 语法。必须使用 `re` 模块。
> - **Pattern vs Match**:
>   - `re.Pattern`: 编译后的正则表达式对象。
>   - `re.Match`: 匹配成功后返回的结果对象（包含捕获组等信息）。

## 4.4 Python 之禅 (The Zen of Python)

最后，我们来聊聊“风格”。在终端输入 `python -c "import this"`，你会看到 Tim Peters 写的 19 条准则。

作为 Python 开发者，有几条必须铭记于心：

1.  **Explicit is better than implicit.** (显式优于隐式)
    - 这就是为什么我们推崇写类型注解，推崇显式的 `self`，而不是隐式的 `this`。
2.  **Flat is better than nested.** (扁平优于嵌套)
    - 尽早 `return`，减少 `if/else` 的层级。
3.  **Readability counts.** (可读性很重要)
    - 代码是写给人看的，顺便给机器运行。

### 实战：海象运算符 (Walrus Operator `:=`)

Python 3.8 引入的 `:=` 是对“扁平化”的一种实践。它允许你在表达式内部赋值。

**Old (Nested)**:

```python
match = pattern.search(text)
if match:
    process(match)
```

**New (Pythonic)**:

```python
# 在 if 条件判断的同时进行赋值
if match := pattern.search(text):
    process(match)
```

这减少了临时变量的作用域泄露，使代码逻辑更紧凑。

---

**本章小结**

这一章我们学习了 Python 的“内功心法”。

1.  **推导式**：用 `[x for x in ...]` 替代 `.map().filter()`，高效且优雅。
2.  **生成器**：用 `(x for x in ...)` 处理大数据流，节省内存。
3.  **正则**：使用 `r""` 原始字符串和 `re.compile`。
4.  **风格**：追求扁平、显式和可读性。

现在，你已经能写出漂亮的 Python 语句了。但是，随着逻辑变得复杂，我们需要更强大的控制流工具。你是否厌倦了 `if-elif-else` 的长链条？

下一章，我们将进入 **逻辑之美**。我们将介绍 Python 3.10 引入的结构化模式匹配 (`match case`)，以及如何利用类型系统做“类型收窄”。这可是 TS 开发者最喜欢的功能之一。

> **思考题**：
> 列表推导式虽然好，但如果逻辑太复杂（比如包含双重循环和多个 `if`），写在一行里会变成“天书”。
> Python 社区对于推导式的可读性有一个不成文的约定：如果一个推导式超过了 **两行** 或者逻辑过于嵌套，应该怎么做？（答案是：放弃推导式，老老实实写 `for` 循环。可读性 > 炫技。）
