...
> **"Knowing is not enough; we must apply. Willing is not enough; we must do."** > **“光知晓是不够的，我们必须应用；光有意愿是不够的，我们必须行动。”**
...
我们在第 9 章讨论了 **IO 边界** 的痛点：外部数据（JSON）是“脏”的，原生类型系统无法防御。而 **Pydantic** 就是为了解决这个问题而生的。而 **FastAPI** 则进一步，将类型系统变成了 API 的路由、文档和验证机制。
...
class UserSchema(BaseModel):
    ...
    # alias 允许前端传 "signup_timestamp"，后端用 "signup_ts" 接收
...
# 在 Pydantic V2 中，推荐使用 model_validate
...
>
> 这就是 Python “代码即数据”哲学的极致体现：你只需要写一次定义，既是类型检查的依据，也是运行时的数据清洗器。
...
# --- 3. 生命周期管理 (Lifespan) ---
# 替代旧版的 @app.on_event，使用标准的 async context manager
@asynccontextmanager
