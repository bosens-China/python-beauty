# 第 12 章：防御性编程与类型安全 (Robustness)

> **"The best safety lies in fear."**
>
> **“最大的安全在于时刻心存忧患。”**
>
> — _威廉·莎士比亚，《哈姆雷特》 (William Shakespeare, Hamlet)_

---

::: tip 💡 上一章答案揭晓
如何编写一个装饰器，既能包裹任意函数，又不丢失原函数的参数类型提示？
你需要使用 `ParamSpec`（参数规范）。它可以“捕获”一个函数的参数列表，并在另一个地方（比如装饰器返回的 wrapper 函数）“重放”它。
我们将在下一章详细讲解这个黑魔法。
:::

写类型注解不仅仅是为了让 IDE 补全，更是为了在**编译期（静态分析期）** 捕获逻辑漏洞。

在 TypeScript 中，你可能习惯了使用 User-Defined Type Guards (`val is Type`) 和 Exhaustiveness Checking (`never`)。好消息是，现代 Python 同样具备这些能力，虽然语法略有不同，但核心理念完全一致。

## 12.1 自定义类型守卫：`TypeGuard`

我们在第 5 章学过，Python 的 `isinstance` 是天然的类型守卫。
`if isinstance(x, str):` 之后，`x` 会自动被收窄为 `str`。

但如果逻辑比较复杂呢？比如检查一个列表是否**全是**字符串？或者检查一个字典是否包含特定的键？`isinstance` 做不到这一点（因为泛型擦除，`isinstance(x, list[str])` 是非法的）。

这时候你需要 **`typing.TypeGuard`** (Python 3.10+)。

```python
from typing import Any, TypeGuard

# 定义一个函数，返回 bool，但告诉静态检查器：
# 如果返回 True，那么入参 val 的类型就是 list[str]
def is_str_list(val: list[Any]) -> TypeGuard[list[str]]:
    # 这里是运行时的检查逻辑
    # 注意：必须确保运行时逻辑和类型注解一致，否则就是欺骗编译器
    return all(isinstance(x, str) for x in val)

def process_items(items: list[Any]):
    # items 此时是 list[Any]

    if is_str_list(items):
        # ✅ 在这个 block 里，IDE 确信 items 是 list[str]
        print(" ".join(items))
    else:
        # ❌ 这里 items 依然是 list[Any]
        print("Not a string list")
```

::: warning ⚠️ 能力越大，责任越大
**警示**：就像 TS 一样，`TypeGuard` 是**开发者对编译器的承诺**。
如果你在 `is_str_list` 里写了错误的逻辑（比如返回了 `True` 但列表里混进了整数），静态检查器依然会无条件相信你。这会导致运行时崩溃。请务必小心编写 Guard 函数。
:::

::: info 📝 TS 开发者便签：`val is Type`

- **TS**: `function isStrList(val: any): val is string[] { ... }`
- **Python**: `def is_str_list(val: Any) -> TypeGuard[list[str]]: ...`
  :::

## 12.2 穷尽性检查：`assert_never`

这是构建健壮系统的杀手锏。
当你使用 `Literal` 或 `Enum` 定义状态时，你希望确保代码处理了**所有**可能的状态。如果未来有人加了一个新状态，静态检查器应该报错提醒。

```python
from typing import Literal, assert_never

type Direction = Literal["left", "right", "up", "down"]

def move(dir: Direction):
    match dir:
        case "left":
            print("Going left")
        case "right":
            print("Going right")
        case "up":
            print("Going up")

        # 假设我们忘记写 "down" 了...

        case _ as unreachable:
            # ❌ 静态检查报错！
            # Argument of type "Literal['down']" cannot be assigned to parameter of type "Never"
            assert_never(unreachable)
```

`assert_never` 的作用是告诉类型检查器：“这行代码永远不应该被执行”。如果有漏网之鱼（比如 `"down"`）流到了这里，它的类型就不是 `Never`，从而触发静态错误。

::: info 📝 TS 开发者便签：The `never` type
这完全等同于 TS 中的 Exhaustiveness Checking 模式：

```typescript
switch (dir) {
  // ... cases ...
  default:
    const _exhaustiveCheck: never = dir; // Error if 'down' is missing
}
```

Python 3.11+ 的 `assert_never` 让这一模式变得标准化了。
:::

## 12.3 错误处理哲学：异常 vs Result 模式

Python 的传统哲学是 **EAFP**（抛出异常）。但异常有一个致命弱点：**它是隐式的**。函数签名里看不到它会抛出什么异常。

在追求高可靠性的代码中（或者受 Rust/Go/FP 影响的 TS 开发者），我们倾向于把错误**显式化**。

### 方式一：联合类型返回 (Union Return)

这是最简单的 Result 模式替代品，利用 Python 的联合类型。

```python
from dataclasses import dataclass

@dataclass
class Success[T]:
    value: T

@dataclass
class Error:
    message: str

# 显式声明：你要么得到成功的结果，要么得到错误对象
def parse_int(s: str) -> Success[int] | Error:
    if s.isdigit():
        return Success(int(s))
    return Error("Not a number")

# 调用方必须显式处理错误，否则类型检查通不过（比如无法访问 value）
result = parse_int("123")

match result:
    case Success(val):
        print(val * 2)
    case Error(msg):
        print(f"Failed: {msg}")
```

### 方式二：使用第三方库

如果你喜欢 TS 的 `fp-ts` 或 Rust 的 `Result<T, E>`，可以使用 Python 的第三方库（如 `returns`），它提供了 `.map()`, `.bind()` 等 Monad 操作。

**建议**：在 Python 中，**不要过度使用 Result 模式**。

- 对于**系统级错误**（网络断连、文件不存在），使用 Python 原生的 Exception。
- 对于**业务级错误**（用户不存在、余额不足），使用 Union Return 或自定义 Exception。过度封装 Result 会让代码变得非 Pythonic。

## 12.4 心智模型：`Optional` 的滥用

`None` (即 TS 的 `null`) 是以十亿美元计的错误。
很多 Python 新手习惯这样写代码：

```python
# ❌ 不好的习惯：滥用 Optional
class User:
    def __init__(self):
        # 先初始化为 None，然后再找机会赋值
        self.name: str | None = None

    def set_name(self, name: str):
        self.name = name

def process(u: User):
    # 每次使用都要判断，不仅烦，而且容易漏
    if u.name is not None:
        print(u.name.upper())
```

这种“先创建空对象，再填值”的模式会导致 `None` 在系统中像病毒一样传播。你永远不知道一个 User 对象到底是“初始化了一半”还是“完整”的。

**防御性建议**：

1.  **构造即完备**：对象在 `__init__` 后必须是完整可用的。
2.  **边界隔离**：如果数据可能缺失（比如 DB 查询结果），在 Controller 层处理完 `None`，传给 Service/Domain 层的数据应当是确定的（Non-Optional）。
3.  **使用哨兵 (Sentinel)**：有时候 `None` 是有效值（比如“用户想把头像置空”），你需要区分“未传参”和“传了 None”。

```python
# 创建一个唯一的对象作为哨兵
MISSING = object()

def update(val: Any = MISSING):
    if val is MISSING:
        print("No value provided")
    elif val is None:
        print("Value is None")
    else:
        print(f"Value is {val}")
```

## 本章小结

防御性编程不仅仅是写 `try...except`，而是利用类型系统构建“编译期防火墙”。

1.  **TypeGuard**: 让你编写自定义的类型收窄逻辑，处理复杂数据结构。
2.  **assert_never**: 确保你的业务逻辑处理了所有可能的分支（枚举/联合类型）。
3.  **Result vs Exception**: 在显式错误处理和 Pythonic 风格之间找到平衡。
4.  **No Optional**: 尽量避免让 `None` 进入你的核心业务逻辑，在边界处消灭它。

掌握了这些，你写的 Python 代码将不再是脆弱的脚本，而是坚固的工程。

接下来，我们将回到 Python 的元编程领域，探讨**装饰器**。但这次，我们将应用本章学到的防御性思维，学习如何编写**不丢失类型信息**的装饰器。这是许多 Python 高级库（如 FastAPI, Celery）的底层秘密。

::: tip 🧠 课后思考
在 TS 中，我们可以写断言函数 `asserts val is string`，这样调用后，**当前作用域后续的**变量类型都会改变。

在 Python 中，`TypeGuard` 能做到这一点吗？
如果我写了一个 `def check_not_none(val) -> TypeGuard[...]:`，然后直接调用 `check_not_none(x)`（不放在 `if` 里），IDE 会把后续的 `x` 视为非空吗？

（提示：目前 Python 还不支持 `asserts` 语法的类型推断，TypeGuard 必须配合 `if` 使用。）
:::
