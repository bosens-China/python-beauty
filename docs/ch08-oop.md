...
> **"To hold, as 'twere, the mirror up to nature."** > **“也就是拿一面镜子去照自然。”**
...
1.  **定义时**：每个实例方法的第一个参数**必须**是 `self`（虽然你可以叫它别的，但千万别这么做）。
2.  **调用时**：Python 会自动把实例作为第一个参数传进去。
    - 调用 `u.greet()` 等同于 `User.greet(u)`。

> **为什么这样设计？**
> Python 的哲学是“显式优于隐式”（Explicit is better than implicit）。`self` 让你清楚地知道这个变量属于实例作用域，而不是局部变量。这也消除了 JS 中 `this` 指向不明的千古难题。
...
inv1.add("Apple")
print(inv2.items) # 输出 ['Apple'] 😱 inv2 被 inv1 污染了！
```
...
### 📝 TS 开发者便签：Access Modifiers

> - **TS**: `private name: string` 是编译时的限制。编译成 JS 后，通常还是可以访问的（除非用新的 `#name` 私有字段）。
> - **Python**: 如果你想表达“私有”，建议用单下划线 `_var`。双下划线 `__var` 在 Python 社区中使用较少，因为它会让调试和继承变得麻烦。
> - **总结**：Python 假设由于大家都是“成年的开发者”（Consenting Adults），会遵守 `_` 开头不要乱动的约定。**秩序来自约定，而非强制。**
...
### 📝 TS 开发者便签：Getters / Setters

> 这与 TS 的语法惊人地相似：
>
> - **TS**: `get area() { return ... }`
> - **Python**: `@property def area(self): ...`
>
> 区别在于 Python 使用装饰器语法。`@property` 是 Python 面向对象中最优雅的特性之一，它允许你随着需求变化，从简单的字段平滑过渡到带有逻辑的方法，而不破坏对外的 API 兼容性。
...
> **思考题**：
> Python 支持多重继承（一个类继承多个父类）。如果 `Class C(A, B)`，而 A 和 B 都有一个 `greet()` 方法，`C().greet()` 到底会调用谁的？
